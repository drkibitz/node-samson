...
module.exports = (function (path, Super, ReadStream, Glob, mimeMagic, Queue) {
     ...
     * @since 1.0
     */
    var event = Object.create(null, /** @lends ResourceReader.event */ {
            /**
             * @event
             * @default "globEnd"
             * @since 1.0
    ...
    function onStreamData(stream, data) {
        this.emit(event.READ_STREAM_DATA, stream, data);
    }
    ...
    function onStreamEnd(stream) {
        this.emit(event.READ_STREAM_END, stream);
    ...
    function onStreamError(stream, exception) {
        this.emit(event.READ_STREAM_ERROR, stream, exception);
        this.destroyStream(stream.path);
    }
    ...
    function onStreamClose(stream) {
        this.emit(event.READ_STREAM_CLOSE, stream);
        this.destroyStream(stream.path);
    }
    ...
    function onGlobEnd(glob, matches) {
        this.emit(event.GLOB_END, matches);
    ...
    function onGlobError(glob, error) {
        this.emit(event.GLOB_ERROR, error);
        onGlob.call(this, glob);
    }
