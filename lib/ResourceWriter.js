 ...
 * @since 1.0
 */
module.exports = (function (path, fs, Super, mkdirp, event, Queue) {
    "use strict";

    // Simulating a Writable Stream with no fs access
    var fdSimulated = 0;
    ...
    function onStreamDrain(stream) {
        this.emit(event.WRITER_STREAM_DRAIN, stream);
    }
    ...
    function onStreamError(stream, exception) {
        this.emit(event.WRITER_STREAM_ERROR, stream, exception);
    ...
    function onStreamClose(stream) {
        this.emit(event.WRITER_STREAM_CLOSE, stream);
        this.destroyStream(stream.path);
    }
    ...
    function newStream(file) {
        ...
        if (stream.writable) {
            this.emit(event.WRITER_STREAM_START, stream);
    ...
    function onMkdirp(file, exception) {
        ...
        if (exception && (exception instanceof Error)) {
            ...
            this.emit(event.WRITER_MKDIRP_END, file);
     ...
     *
     * @fires module:node-module/lib/event.WRITER_MKDIRP_END
     * @fires module:node-module/lib/event.WRITER_STREAM_START
     * @fires module:node-module/lib/event.WRITER_STREAM_DRAIN
     * @fires module:node-module/lib/event.WRITER_STREAM_ERROR
     * @fires module:node-module/lib/event.WRITER_STREAM_CLOSE
     * @fires module:node-module/lib/event.WRITER_END
     * @since 1.0
     */
    function ResourceWriter(emitter, filterFilePath, filterFileData) {
        // callSuper
        Super.call(this);
