...
module.exports = (function (path, fs, mimeparse, TemplateSettings, Template) {
    ...
    function prepareData(mimeType, file, data) {
        ...
        case TemplateSettings.validationCode.BROKEN_ITERATE:
     ...
     * @private
     */
    Object.defineProperties(ResourceProcessor.prototype, /** @lends ResourceProcessor# */ {
        /**
         * @type {uint}
         * @default 0
         * @since 1.0
         ...
         * @since 1.0
         */
        filterDefDescriptor : {value : null, writable: true},
         ...
         */
        processFile : {
            value : function processFile(mimeType, file, data, def, vars) {
                var i, l, keys, descriptor, tpl;

                tpl = this.getActiveTpl(file);
                // If we don't have a template yet, create one now
                if (null === tpl) {
                    ...
                    // Provide a last ditch effort to modify ot change def descriptor
                    if ('function' === typeof this.filterDefDescriptor) {
                        descriptor = this.filterDefDescriptor(descriptor);
                    }
                    def = Object.create(ResourceProcessor.def, descriptor);

                    // Now create a new Template with a def using descriptor
                    tpl = this.tpls[file] = new Template(path.extname(file).substr(1), def);
                }
    ...
    // Module provides
    return Object.defineProperties(ResourceProcessor, /** @lends ResourceProcessor */ {
        /**
         * @namespace
         * @readonly
         * @since 1.0
         */
        def : {
            value : Object.create(process.env, /** @lends ResourceProcessor.def */ {
                /**
                 * @readonly
                 * @type {Object}
                 * @default new Object()
